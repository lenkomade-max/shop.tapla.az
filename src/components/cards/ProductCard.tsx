    ...
    >
      ...
      <div className="relative w-full aspect-[4/5] bg-neutral-50 overflow-hidden rounded-t-xl">
        <Link
          ...
          onClick={() => sessionStorage.setItem('tapla_scroll', String(window.scrollY))}
        >
          <Image
        ...
        {/* Quick view: always visible on mobile, on hover on desktop (no dimming) */}
        <div className="md:hidden absolute inset-0 z-10 pointer-events-none flex items-end justify-center pb-4">
          <button
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
        ...
        <div className="hidden md:flex absolute inset-0 z-10 pointer-events-none opacity-0 transition-opacity duration-300 group-hover:opacity-100 items-end justify-center pb-4">
          <button
            onClick={(e) => {
              e.preventDefault();
              e.stopPropagation();
          ...
          <div className="flex items-center justify-between gap-2 mb-1.5">
            <div className="flex items-baseline gap-1.5 min-w-0 shrink-0 flex-wrap">
              {hasDiscount ? (
                ...
                <span className="text-base sm:text-xl font-bold tracking-tight text-neutral-900">
                  {product.price.toFixed(2)} ₼
                </span>
              )}
              {hasDiscount && (
                <span className="text-sm sm:text-base text-neutral-500 line-through shrink-0">
